...
> 当前为 Alpha。生产模式默认关闭全部测试 fixture、使用空白真实档案，并允许浏览器在逐岗位授权后执行正式投递。默认存储仍为内存且没有多用户账号体系，不要在无额外认证时把真实简历服务直接暴露给不受信任的公网用户。
...
详细的环境矩阵、API 冒烟、隔离 fixture 旅程、失败用例和发布回归步骤见 [测试手册](docs/TESTING.md)。正式环境必须保持 `ENABLE_TEST_FIXTURES=false`。
