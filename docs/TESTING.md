...
准备：用独立测试进程启动 fixture，再加载 `browser_extension`：

```powershell
$env:ENABLE_TEST_FIXTURES="true"
$env:APP_ENV="test"
$env:AGENT_PORT="18011"
.\.venv\Scripts\python.exe -m resume_campaign_agent
```

生产服务必须保持 `ENABLE_TEST_FIXTURES=false`。测试扩展需构建为指向该独立端口，不能复用正式扩展。
...
此安全预演只用于自动化回归。正式验收应使用用户本人真实资料，并在不可逆最终提交按钮前停止，等待用户针对当前岗位确认。
